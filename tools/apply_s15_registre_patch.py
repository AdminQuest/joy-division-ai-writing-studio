#!/usr/bin/env python3
"""
Applique le patch canonique S15 au registre principal.
...
PATCH = REPO_ROOT / "sources" / "deluca_manchester_punk_threshold" / "registre_patch_s15.json"
...
def main() -> None:
    ...
    if patch.get("id") != "S15":
        raise SystemExit("Le patch ne cible pas S15")

    updated = False
    for i, entry in enumerate(registre):
        if isinstance(entry, dict) and entry.get("id") == "S15":
            merged = dict(entry)
            merged.update(patch)
            registre[i] = merged
            updated = True
