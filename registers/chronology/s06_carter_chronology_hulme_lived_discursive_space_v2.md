# S06 — Compléments au registre chronologique — Hulme, inner city et récits habitants

```yaml
id: CHRONO-S06-HULME-LIVED-DISCURSIVE-SPACE-V2
source_id: S06
source_label: "S06 — Carter, Youth, race and the inner-city estate, 2021/2023"
type_unite: registre_chronologie
statut: integration_directe
```

```yaml
events:
  - id: CHR-S06-001
    date_precision: annee
    categorie: contexte
    date: 1969
    precision_date: annee
    event: "Shaima arrive à Hulme avec sa famille alors qu’elle est âgée d’environ un an."
    type: trajectoire_residentielle
    location: Hulme
    sources:
      - S06
    certainty: strong
    related_atoms:
      - S06-A010

  - id: CHR-S06-002
    date_precision: annee
    categorie: contexte
    date: 1970
    precision_date: annee
    event: "Lee-Ann naît à Hulme, où elle grandit dans un environnement familial et communautaire diversifié."
    type: trajectoire_residentielle
    location: Hulme
    sources:
      - S06
    certainty: strong
    related_atoms:
      - S06-A009

  - id: CHR-S06-003
    date_precision: annee
    categorie: contexte
    date: 1975
    precision_date: annee
    event: "La mort d’un garçon de cinq ans après une chute depuis un balcon de Hulme conduit à des revendications de relogement des familles avec enfants."
    type: evenement_urbain
    location: Hulme Crescents
    sources:
      - S06
    certainty: strong
    related_atoms:
      - S06-A002
      - S06-A005

  - id: CHR-S06-004
    date_precision: annee
    categorie: contexte
    date: 1977
    precision_date: annee
    event: "Le Hulme People’s Rights Centre décrit Hulme comme exemple extrême de deprivation et d’urban redevelopment."
    type: publication_locale
    location: Hulme
    sources:
      - S06
    certainty: strong
    related_atoms:
      - S06-A003

  - id: CHR-S06-005
    date_precision: annee
    categorie: contexte
    date: 1978
    precision_date: annee
    event: "Le Manchester and Salford Inner City Partnership identifie la jeunesse de l’inner city comme problème prioritaire."
    type: politique_urbaine
    location: Manchester / Salford
    sources:
      - S06
    certainty: strong
    related_atoms:
      - S06-A003
      - S06-A005

  - id: CHR-S06-006
    date_precision: annee
    categorie: reception_posthume
    date: 1985
    precision_date: annee
    event: "Le Manchester Evening News publie le reportage « Horrors of the concrete jungle » sur Hulme."
    type: representation_mediatique
    location: Hulme
    sources:
      - S06
    certainty: strong
    related_atoms:
      - S06-A003
      - S06-A004

  - id: CHR-S06-007
    date_precision: annee
    categorie: reception_posthume
    date: 1986
    precision_date: annee
    event: "Hulme présente le plus fort taux de chômage des jeunes à Manchester selon les sources mobilisées par Carter."
    type: contexte_social
    location: Hulme
    sources:
      - S06
    certainty: strong
    related_atoms:
      - S06-A003

  - id: CHR-S06-008
    date_precision: annee
    categorie: reception_posthume
    date: 1990
    precision_date: annee
    event: "Le Hulme Views Project publie des témoignages et autoportraits d’habitants de Hulme."
    type: projet_communautaire
    location: Hulme
    sources:
      - S06
    certainty: strong
    related_atoms:
      - S06-A006
      - S06-A008

  - id: CHR-S06-009
    date_precision: annee
    categorie: reception_posthume
    date: 1993
    precision_date: annee
    event: "Démolition des Hulme Crescents, présentée dans la presse comme signal d’un nouveau départ pour le quartier."
    type: demolition
    location: Hulme Crescents
    sources:
      - S06
    certainty: strong
    related_atoms:
      - S06-A002

  - id: CHR-S06-010
    date_precision: annee
    categorie: reception_posthume
    date: 1995
    precision_date: annee
    event: "La presse locale présente le redevelopment de Hulme comme inner-city rebirth."
    type: representation_mediatique
    location: Hulme
    sources:
      - S06
    certainty: strong
    related_atoms:
      - S06-A002

  - id: CHR-S06-011
    date_precision: annee
    categorie: reception_posthume
    date: 2021
    precision_date: annee
    event: "Publication en ligne par Cambridge University Press de l’article d’Isabelle Carter sur Hulme."
    type: publication
    location: Cambridge University Press
    people:
      - Isabelle Carter
    sources:
      - S06
    certainty: strong
    related_atoms:
      - S06-A001

  - id: CHR-S06-012
    date_precision: annee
    categorie: reception_posthume
    date: 2023
    precision_date: annee
    event: "Parution bibliographique de l’article de Carter dans Urban History, vol. 50, p. 248-263."
    type: publication
    sources:
      - S06
    certainty: strong
    related_atoms:
      - S06-A001
```
